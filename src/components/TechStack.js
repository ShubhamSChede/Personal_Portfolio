...
const TechStack = () => {
  ...
  useEffect(() => {
    ...
    const initialMinimizeState = {};
    techData.sections.forEach((section, index) => {
      initialMinimizeState[index] = false;
    });
    setMinimized(initialMinimizeState);
  ...
  const toggleMinimize = (sectionIndex) => {
    setMinimized(prev => ({
      ...prev,
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center gap-3 mb-12 stack-section opacity-0 translate-y-8 transition-all duration-700 ease-out">
        ...
        {techData.sections.map((section, sectionIndex) => (
          ...
          >
            <div 
              ...
              onClick={() => toggleMinimize(sectionIndex)}
            >
              <h3 className="text-5xl font-bold mb-8 text-gray-500">{section.title}</h3>
              <button className="text-gray-500 hover:text-white transition-colors p-2 rounded-full">
                {minimized[sectionIndex] ? (
            ...
            <div 
              className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 transition-all duration-500 ease-in-out overflow-hidden ${
                minimized[sectionIndex] ? 'max-h-0 opacity-0 mt-0' : 'max-h-[2000px] opacity-100 mt-4'
              }`}
            >
